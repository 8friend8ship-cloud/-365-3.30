...
function runBible4DailyFrontDelivery_() {
  ...
  try {
    ...
    bible4AppendLog_(ss, 'FULL_CHAIN', 'DONE', 'Content→Audio→App Delivery 실제 출력 확인', evidence);
    return evidence;
  } catch (error) {
...
function bible4SheetEvidence_(ss, sheetName) {
  ...
  if (!sh) return { sheet: sheetName, exists: false, lastRow: 0, updatedToday: false };
  ...
  if (lastRow >= 2) {
    ...
    sample = sh.getRange(start, 1, lastRow - start + 1, Math.min(lastColumn, 64)).getDisplayValues();
  }
  var today = Utilities.formatDate(new Date(), Session.getScriptTimeZone() || 'Asia/Seoul', 'yyyy-MM-dd');
  var flat = sample.map(function(r) { return r.join(' | '); }).join('\n');
  var updatedToday = flat.indexOf(today) !== -1;
  ...
  return {
    ...
    containsBrowserToken: flat.indexOf('token=') !== -1
  };
}
