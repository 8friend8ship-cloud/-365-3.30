 ...
 * - Gemini 보강은 별도 단계에서만 사용하고, 이 브리지는 생산 큐가 AI 비용/쿼터 때문에
 *   멈추지 않도록 한다.
 */
...
function bible365BuildABCDEFromQueens() {
  const ss = SpreadsheetApp.openById(
    PropertiesService.getScriptProperties().getProperty('BIBLE365_MAIN_SPREADSHEET_ID') ||
      B365_ABCDE_MAIN_SHEET_ID
  );
  ...
  if (!sourceSheet || !abideSheet || !dbMapSheet) {
    throw new Error('B365_ABCDE_REQUIRED_SHEET_MISSING');
  }
  ...
  for (let r = 1; r < sources.length; r++) {
    ...
    const mapped = {
      ABIDE_ID: abideId,
      SOURCE_ID: sourceId,
      A_CODE: `핵심 믿음·압박: ${theme || '현재 주제'}에서 ${conflict || '내적 갈등이 생긴다'}`,
      B_CODE: `반복 행동·반응: ${scene || '갈등 장면에서 익숙한 반응을 반복한다'}`,
      C_CODE: `숨은 비용: ${emotion || '긴장'}이 누적되어 관계와 회복의 비용이 커진다`,
      D_CODE: `관성·고착: 문제를 알아도 기존 해석과 반응을 유지해 같은 갈등을 반복한다`,
      E_CODE: `선택의 대가: ${hook || '다음 선택에서 얻는 것과 잃는 것을 함께 본다'}`,
      ABIDE_KEY: `AUTOABCDE|${sourceId}`,
      STATUS: 'AUTO_READY',
      CREATED_AT: new Date(),
    };
  ...
  if (newRows.length) {
    abideSheet.getRange(abideSheet.getLastRow() + 1, 1, newRows.length, newRows[0].length).setValues(newRows);
  }
...
function linkABCDEIntoDbMap_(sheet, existingBySource) {
  ...
  if (idx.SOURCE_ID === undefined || idx.ABIDE_ID === undefined) {
    throw new Error('B365_ABCDE_DBMAP_COLUMNS_MISSING');
  }
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    if (sourceId && !current && abideId) {
      sheet.getRange(r + 1, idx.ABIDE_ID + 1).setValue(abideId);
      changed++;
    }
  }
  return changed;
...
function inspectBible365ABCDEBridge() {
  ...
  const readyDbWithAbide = db.slice(1).filter(r =>
    str_(r[dIdx.STATUS]) === 'READY' && Boolean(str_(r[dIdx.ABIDE_ID]))
  ).length;
